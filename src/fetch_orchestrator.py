"""fetch 后台编排段文件（T4.2，由 src/routes/fetch.py 纯机械拆分）。

本文件不是独立模块：由 routes/fetch.py 用 exec 装载进它自己的
全局命名空间执行，以保持 monkeypatch.setattr(routes.fetch, ...)
与模块级可变状态（_fetch_running/_fetch_active_runs 等）的原有语义。
禁止直接 import fetch_orchestrator。仿照 src/remote_db/__init__.py 的段文件约定。
"""
# ── Module-level state ──────────────────────────────────────
            ...
            try:
                ...
                if highlights_read_model_refresh_enabled:
                    try:
                        ...
                        media_result = reddit_media_completion.complete_highlighted_reddit_media()
                        print(
                            f"[reddit-media] completion after highlights refresh: {media_result}",
                            flush=True,
                        )
                    except Exception as media_exc:
                        print(
                            f"[reddit-media] completion exc: {media_exc!r}",
                            flush=True,
                        )
...
def _run_platform_all(platform, *, output_root=None, env=None):
    ...
    if platform == 'twitter':
        ...
        if not _is_platform_enabled('bilibili'):
            print("[fetch] bilibili is disabled in config, skipping")
            return
...
def _run_source_fetch_step(platform, source, *, output_root=None, env=None):
    ...
    if platform == 'xiaohongshu' and source == 'recommend':
        _run_to_file([CLI['xhs'], 'feed', '--json'],
            ...
            env=command_env)
        return True
    if platform == 'bilibili' and not _is_platform_enabled('bilibili'):
        print("[fetch] bilibili is disabled in config, skipping")
        return True
